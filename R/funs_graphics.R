...
theme_ongo <- function(base_size = 11, base_family = "IBM Plex Sans Condensed", prior = FALSE) {
  ret <- theme_bw(base_size, base_family) +
    theme(panel.background = element_rect(fill = "#ffffff", colour = NA),
      title = element_text(size = rel(1), family = "IBM Plex Sans Condensed", face = "bold"),
      plot.subtitle = element_text(size = rel(0.8),
        family = "IBM Plex Sans Condensed", face = "plain"),
      plot.caption = element_text(margin = margin(t = 10), size = rel(0.75),
        family = "IBM Plex Sans Condensed", face = "plain"),
...
abs_comma <- function(x) {
  scales::label_comma(style_negative = "minus")(abs(x))
}
